...
RSpec.describe UsersController, type: :controller do
  ...
  describe "Actions" do
    ...
    describe "GET Edit" do
      ...
      it "should redirect to root if user has no permission" do
        sign_in_mock_user(employee_user)
        get :edit, params: { id: common_user.id }
        expect(response).to redirect_to(root_path)
      end
    ...
    describe "POST Create" do
      ...
      it "should redirect to root if user has no permission" do
        sign_in_mock_user(employee_user)
        b_user = build(:user)
        post :create, params: { user: b_user.attributes }
    ...
    describe "PUT Update" do
      ...
      it "should redirect to root if user has no permission" do
        sign_in_mock_user(employee_user)
        put :update, params: {id: common_user.id, user: { name: "Name Test" } }
        expect(assigns(:resource)).to eq(nil)
      end
    ...
    describe "DELETE Destroy" do
      ...
      it "should redirect to root if user has no permission" do
        sign_in_mock_user(employee_user)
        delete :destroy, params: { id: common_user.id }
        expect(response).to redirect_to(root_path)
      end
